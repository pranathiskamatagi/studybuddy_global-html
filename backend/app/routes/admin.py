...
def live_activity():
    ...
    from app.sockets import get_searching_user_ids, session_participants

    # A session only counts as live while someone is actually inside its
    # chat room right now - being logged in elsewhere in the app doesn't.
    live_session_ids = set(session_participants.values())
    ...
    for s in ongoing:
        if s.mode == 'group':
            ...
            if not members:
                continue
            names = [db.session.get(User, m.user_id).fullname for m in members if db.session.get(User, m.user_id)]
            if s.id not in live_session_ids:
                continue
        else:
...
@admin_bp.get('/conversation-summary')
@jwt_required()
def conversation_summary():
    # WHO talked with whom, how many times, and when - never the messages
    # themselves. A "conversation" is a session where somebody actually
    # sent a message.
    denied = _forbid_non_admin()
    if denied:
        return denied
    ...
    def name_of(user_id):
        ...
        return user.fullname if user else 'Unknown'

    groups = {}
    for s in sessions:
        if s.mode == 'group':
            sender_ids = sorted(
                sid for (sid,) in db.session.query(Message.sender_id)
                .filter(Message.session_id == s.id).distinct().all()
            )
            key = ('group',) + tuple(sender_ids)
            names = [name_of(i) for i in sender_ids]
            label = 'Group: ' + ', '.join(names)
        else:
            ids = sorted(i for i in (s.learner_id, s.partner_id) if i)
            key = ('pair',) + tuple(ids)
            label = ' & '.join(name_of(i) for i in ids)
        entry = groups.setdefault(key, {'label': label, 'conversations': []})
    ...
    people = sorted(
        (
            {'label': g['label'], 'count': len(g['conversations']), 'conversations': g['conversations']}
            for g in groups.values()
        ),
        key=lambda g: g['conversations'][0]['startedAt'],
        reverse=True,
    )
    return jsonify(total=len(sessions), people=people)
